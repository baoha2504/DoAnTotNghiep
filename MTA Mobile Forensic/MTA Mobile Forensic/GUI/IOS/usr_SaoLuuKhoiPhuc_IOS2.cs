...
{
    public partial class usr_SaoLuuKhoiPhuc_IOS2 : UserControl
    {
        api api = new api();
        string query = "";
        string fullFilePath = "";
        ...
        public usr_SaoLuuKhoiPhuc_IOS2()
        {
            InitializeComponent();
        ...
        private async void btnSaoLuu_Click(object sender, EventArgs e)
        {
            if (txtPathBackup.Text != string.Empty)
            {
                ...
                {
                    command = $"--udid {DeviceInfo.serialDevice}";
                }
                try
                {
                    query = $"iMazing-CLI.exe --backup-device {command} --backup-path \"{fullFilePath}\"";
            ...
            await Task.Run(() =>
            {
                ...
                process.ErrorDataReceived += (sender, args) =>
                {
                    if (!string.IsNullOrEmpty(args.Data))
                    {
                        ...
                        this.Invoke(new Action(() =>
                        {
                            if (txtBackup.Text.ToLower().Contains("ERROR"))
                            {
                ...
                this.Invoke(new Action(() =>
                {
                    ...
                    if (txtBackup.Text.ToLower().Contains("backup successful"))
                    {
        ...
        private async void btnKhoiPhuc_Click(object sender, EventArgs e)
        {
            if (txtPathRestore.Text != string.Empty)
            {
                ...
                query = $"iMazing-CLI.exe --restore-backup {command} --backup-path \"{fullFilePath_Restore}\"";
